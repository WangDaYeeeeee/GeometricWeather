...
internal class SnackbarManager private constructor() {
    ...
    fun dismiss(callback: Callback, event: Int) {
        synchronized(mLock) {
            when {
                isCurrentSnackbar(callback) -> cancelSnackbarLocked(mCurrentRecord!!, event)
                isNextSnackbar(callback) -> cancelSnackbarLocked(mNextRecord!!, event)
                else -> Unit
            }
        }
    }
