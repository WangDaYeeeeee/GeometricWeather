...
| 测试 | JUnit5 Jupiter + mockk；JVM Compose UI Test（`:feature:settings` `createComposeRule` + Robolectric/`AndroidJUnit4`，vintage 仅该模块）；`:app` `androidTest` 仍空 | JUnit5 + Compose UI Test |
...
- [x] 拆分 `:data`（网络/数据库/API 服务）— Room（`db/`、`DatabaseHelper`、`FileUtils` + `city_list.txt`）**以及** Retrofit weather JSON/`apis`、`WeatherService*`/`converters`/`WeatherServiceSet`/`WeatherHelper`、Hilt `weather.di.ApiModule` + `location.di.ApiModule`、Baidu IP `BaiduIPLocationApi`/`BaiduIPLocationResult`/`BaiduIPLocationService`、`LocationService`/`LocationHelper`/`LocationException`。`:data` 自带 `buildConfigField`（与 `:app` 同一套 gradle 属性）；运行时 key/语言/降水单位/天气源/定位源/Baidu IP AK 走 `WeatherProviderSettings`（读 Settings 同一套 SharedPreferences，不依赖 `:presentation`）。OkHttp/`RetrofitModule`/Gzip/TLS 在 `:data`；`:app` 经 `NetworkDebugConfig`（debuggable → HttpLoggingInterceptor BODY）与 `NetworkExceptionReporter`（Bugly/`src_nobugly` no-op）注入，避免 `:data` 依赖 `GeometricWeather` 或 Bugly SDK。Flavor 定位实现（Baidu/AMap/GMS/`src_*` stubs）仍在 `:app`，由 `FlavorLocationFactory` 绑定。
- [x] 拆分 `:domain`（模型/用例）— `common/basic/models`（天气/地点/options）。Android library（Context/`R`/Parcelable，不是纯 JVM）。**第一批用例**（无 Room/Retrofit）：`LocationWeatherStore` 接口 + `LoadLocationsWithWeatherUseCase` / `HydrateWeatherCacheUseCase` / `LoadAllLocationsWithWeatherUseCase` / `DeleteLocationUseCase` / `CacheRequestedWeatherUseCase`；`WeatherRequester` / `LocationSearcher` 由 `:data` `WeatherHelper` 实现。实现：`DatabaseLocationWeatherStore` 包 `DatabaseHelper`。`MainActivityRepository` / `SearchActivityRepository` / `PollingUpdateHelper` 走新类型。仍在 helpers / Activity 直连 `DatabaseHelper`：中文城市表（`CaiYunWeatherService`）、Alert/Allergen/Daily、设置天气源、Widget/RemoteViews/wallpaper/`TileService`/`ShortcutsHelper`。`Temperature` 仍通过 `exchangeDayNightTempEnabled` 钩子读设置。
- [x] 拆分 `:feature`（按功能模块，如 main/search/settings）— `:feature:search`（SearchActivity + compose + SearchActivityRepository）、`:feature:settings`（settings/about/preview-icon Activities + compose 屏）、**`:feature:main`**（MainActivity + Home `AndroidView` + WeatherView + daily/alert/allergen；`MainActivityViewModel` 经 `MainWeatherRepository` / `MainBackgroundBridge`；**`MainActivityRepository` 在 `:feature:main`** `@Binds` `MainWeatherRepository`）。共享壳层抽到 **`:presentation`**（`GeoActivity` / `ThemeManager` / `weatherThemeDelegate` / weatherView / `GeometricWeatherTheme` / `InAppRoute` / `SettingsManager` / icon `ResourceProvider`），`:core` 仍不依赖 `:domain`。Widget/polling 经 `SettingsAppCallbacks` 与 `MainBackgroundViews` 留在 `:app`。**`LocationHelper` 在 `:data`**；flavor `src/src_*` location services（Baidu/AMap/`AndroidLocationService`）仍在 `:app`，经 `FlavorLocationFactory` 注入。定位通知 channel/id 在 `:core` `LocationNotificationChannels`。AppWidget/RemoteViews/live wallpaper 仍在 `:app`。
...
- **JUnit5 迁移（本轮）**：已从 JUnit4 + PowerMock 迁到 JUnit5 Jupiter；PowerMock **已全部移除**。原 PowerMock 静态 `TextUtils` mock（`CardDisplayTest` / `DailyTrendDisplayTest`）改为 mockk `mockkStatic`；`Resources`/`Context` 用 mockk。不再需要 JDK `--add-opens`。Robolectric 4.14.1 无官方 Jupiter runner。Compose UI Test（JVM）：`:feature:settings` 对纯 composable（`SectionHeader` / `PreferenceView`，假数据、无 Hilt/网络）用 `createComposeRule` + `onNodeWithText`/`assertIsDisplayed`。`createComposeRule` 是 JUnit4 `TestRule`，故该 **一个** 类用 `AndroidJUnit4` + Robolectric（`@GraphicsMode(NATIVE)`，sdk 28）。`junit-vintage-engine` 仅 `:feature:settings` `testRuntimeOnly`，以便 JUnit Platform 同时跑 Jupiter 与这一条；**未** 加 PowerMock，**未** 给 `:app` 或其它模块加 vintage。试过 `tech.apter` `RobolectricExtension` + `runComposeUiTest`：测试方法被放到 Robolectric 主线程后 `Espresso.onIdle` 死锁，已弃用。任务 **`:feature:settings:testDebugUnitTest`**。全屏仍需 Hilt + instrumented Activity，未测。`:app` 仍 `androidTestImplementation(compose-ui-test-junit4)`，`src/androidTest` 仍空（无 emulator）。`:core` 无 `src/test`。`GreenDaoToRoomMigrationTest` 仍只测列名映射。
