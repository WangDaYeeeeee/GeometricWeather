    ...
    private void initPreferences() {
        ...
        if (getBuildFlavor().contains("fdroid")) {
            ...
            for (int i = 0; i < chineseSource.getEntries().length; ++i) {
                if (OptionMapper.getWeatherSource((String) chineseSource.getEntryValues()[i]) != WeatherSource.CN
                        && OptionMapper.getWeatherSource((String) chineseSource.getEntryValues()[i]) != WeatherSource.CAIYUN) {
        ...
        if (getBuildFlavor().contains("fdroid")) {
            ...
            for (int i = 0; i < locationService.getEntries().length; ++i) {
                if (OptionMapper.getLocationProvider((String) locationService.getEntryValues()[i]) != LocationProvider.AMAP
                        && OptionMapper.getLocationProvider((String) locationService.getEntryValues()[i]) != LocationProvider.BAIDU
                        && OptionMapper.getLocationProvider((String) locationService.getEntryValues()[i]) != LocationProvider.BAIDU_IP) {
