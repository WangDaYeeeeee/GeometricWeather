    ...
    private fun refreshBackgroundViews(resetBackground: Boolean, locationList: List<Location>?) {
        if (resetBackground) {
            AsyncHelper.delayRunOnIO({
                widgetUpdateGateway.resetAllBackgroundTasks(this, false)
            }, 1000)
        }
        locationList?.let {
            if (it.isNotEmpty()) {
                AsyncHelper.delayRunOnIO({
                    widgetUpdateGateway.updateWidgetsIfNecessary(this, it[0])
                    widgetUpdateGateway.updateNotificationIfNecessary(this, it)
                    widgetUpdateGateway.updateWidgetsIfNecessary(this, it)
                }, 1000)
