...
class CloudImplementor @SuppressLint("SwitchIntDef") constructor(
    ...
    private lateinit var mClouds: Array<Cloud>
    private lateinit var mStars: Array<Star>
    private val mThunder: Thunder?
    private val mRandom = Random()

    @IntDef(TYPE_CLOUD, TYPE_CLOUDY, TYPE_THUNDER, TYPE_FOG, TYPE_HAZE)
    annotation class TypeRule
    ...
    private class Cloud(
        ...
        private val mInitCX = centerX
        private val mInitCY = centerY
        ...
        var scaleRatio: Float = scaleRatio
        var moveFactor: Float = moveFactor
    ...
    private class Thunder {
        ...
        private fun computeFrame() {
            alpha = if (progress < duration) {
                when {
                    progress < 0.25 * duration -> (progress / 0.25 / duration).toFloat()
                    progress < 0.5 * duration -> (1 - (progress - 0.25 * duration) / 0.25 / duration).toFloat()
                    progress < 0.75 * duration -> ((progress - 0.5 * duration) / 0.25 / duration).toFloat()
                    else -> (1 - (progress - 0.75 * duration) / 0.25 / duration).toFloat()
                }
            } else {
                0f
            }
        ...
        if (type == TYPE_FOG || type == TYPE_HAZE) {
            ...
            if (type == TYPE_FOG) {
                cloudColors = if (daylight) {
                    intArrayOf(0xFF717DA0.toInt(), 0xFF717DA0.toInt(), 0xFF717DA0.toInt())
                } else {
                    intArrayOf(
                        Color.rgb(85, 99, 110),
                ...
                cloudAlphas = if (daylight) {
                    floatArrayOf(0.1f, 0.1f, 0.1f)
                } else {
                    floatArrayOf(0.4f, 0.6f, 0.4f)
                }
            } else {
                cloudColors = if (daylight) {
                    intArrayOf(0xFFAC9D82.toInt(), 0xFFAC9D82.toInt(), 0xFFAC9D82.toInt())
                } else {
                    intArrayOf(
            ...
            when (type) {
                ...
                TYPE_THUNDER -> {
                    cloudColors = if (daylight) {
                        intArrayOf(0xFFBCADC1.toInt(), 0xFFBCADC1.toInt())
                    } else {
                        intArrayOf(
                            Color.rgb(43, 30, 66),
                            Color.rgb(43, 30, 66),
            ...
            val cloudAlphas = floatArrayOf(0.40f, 0.10f)
            ...
            if (daylight) {
                ...
                val r = Random()
                val canvasSize = Math.sqrt(Math.pow(viewWidth.toDouble(), 2.0) + Math.pow(viewHeight.toDouble(), 2.0)).toInt()
    ...
    private fun initialize(clouds: Array<Cloud>, stars: Array<Star>?) {
        mPaint = Paint()
        mPaint.style = Paint.Style.FILL
        mPaint.isAntiAlias = true
        mClouds = clouds
        mStars = stars ?: emptyArray()
    }
    ...
    override fun draw(
        ...
        if (scrollRate < 1) {
            val thunder = mThunder
            if (thunder != null) {
                canvas.drawColor(
                    Color.argb(
                        ((1 - scrollRate) * thunder.alpha * 255 * 0.66).toInt(),
                        thunder.r,
                        thunder.g,
                        thunder.b
                    )
    ...
    companion object {
        ...
        fun getThemeColor(context: Context, @TypeRule type: Int, daylight: Boolean): Int {
            return when (type) {
                TYPE_CLOUDY -> if (daylight) 0xFF9DAFC1.toInt() else 0xFF263238.toInt()
                TYPE_CLOUD -> if (daylight) 0xFF00a5d9.toInt() else 0xFF222d43.toInt()
                TYPE_THUNDER -> if (daylight) 0xFFB296BD.toInt() else 0xFF231739.toInt()
                TYPE_FOG -> if (daylight) 0xFFA3AEC2.toInt() else 0xFF4F5D68.toInt()
                TYPE_HAZE -> if (daylight) 0xFFE1C899.toInt() else 0xFF6c5c49.toInt()
